...
get.motifs <- function( biclusters, motif.clusters, genes, positions, window=10,
                       expand.motif.cluster.clusters=T, op.shift=F, ... ) {
  if ( ! missing( biclusters ) ) {
    ...
    if ( positions[ 1 ] == "choose" ) { ## get it from a plot (probably output from plot.promoter.architecture())
      ...
      for ( gene in positions ) {
        tmp <- e$get.sequences( gene, seq.type='upstream meme', op.shift=op.shift,
                               distance=e$motif.upstream.search[[ 1 ]], filter=F, ... )
        if ( is.null( tmp ) ) next
...
get.motif.positions <- function( motifs="ALL", seqs=e$genome.info$all.upstream.seqs[[ 1 ]],
  ...
  if ( motifs != "ALL" ) {
    ...
    scans <- p.scans[ J( c( bi, bi ), c( mo, -mo ) ) ] ##p.scans[ J( bi, mo ) ] ##, names( seqs ) ) ]
  } else {
    scans <- p.scans
  }
...
get.pssm.scans <- function( motifs="ALL", seqs=e$genome.info$genome.seqs,
  ...
  setnames( tab, 'gene', 'pvals', 'posns', 'mots', 'bic' )
...
get.motif.coding.fracs <- function( motifs='ALL', verbose=F, p.cutoff=1e-6 ) {
  ...
  if ( motifs == 'ALL' ) {
    ...
    bi <- as.integer( sapply( mots, "[", 1 ) )
    mo <- as.integer( sapply( mots, "[", 2 ) )
    scans <- scans[ J( c( bi, bi ), c( mo, -mo ) ) ]  ## this speeds it up a lot
  }
...
plot.genes.in.region <- function( center, where, window, new.plot=T, yoff=0, yscale=1, org=NA, gene.names=T,
...
plot.promoter.architecture <- function( gene, window=125, shift=75, e.value.cutoff=Inf, p.value.cutoff=1e-5,
                                       ...
                                       type=c('mast','meme','fimo'), dont.plot=F, count.all=F, ... ) {
  ...
  if ( type == 'mast' ) {
    scans <- pssm.scans[ J( c( bi, bi ), c( mo, -mo ), chr ) ] ##subset( pssm.scans, bic %in% bi )
    scans <- scans[ ! is.na( scans$posns ), ]
  ...
  yr <- e$viewPssm( mat, scale.e=apply( mat, 1, sum ), no.axis.labels=T, ... ) ##, border=NA ) ## can add 'border=NA' arg. to the
  ...
  if ( ncol( counts ) > 0 ) {       ## plot.promoter.architecture.2() call.
    ...
    legend( 'topleft', legend=paste( colnames( counts ), leg.count ), lwd=3,
           col=1:ncol( counts ), lty=(1:ncol( counts ) %/% 8) + 1, cex=0.6, horiz=F, trace=F, seg.len=5 )
  }
  ##plot( st.st, c( -2, 2 ), typ='n' ) ##, mar=c(0,0,0,0), xaxs='i', yaxs='i' )
  try( plot.genes.in.region( mean( st.st[1:2] ), chr, diff( range( st.st[1:2] ) ), new=T, yscale=0.5 ) )
  mat3 <- mat * 0
...
cluster.motifs <- function( cluster.option, motifs='ALL', min.gene.overlap=1, ## NA for all-vs-all motif comparison
                           ...
                           in.tt.out=NULL,
                           mcl.I=3.0, mcl.cmd='./progs/mcl-10-201/local/bin/mcl mcltemp2 -o %s --abc -I %.1f -v all -te 3 -S 200000 -scheme 7 --analyze=y -pi 4.0', ... ) {
  ...
  if ( is.null( motifs ) || motifs == 'ALL' ) {
    ...
    if ( include.bad == FALSE && exists( 'coding.fracs' ) )
      motifs <- names( which( coding.fracs$all.fracs < coding.fracs$mean.fracs - 0.01 ) )
  ...
  if ( is.null( tt.out ) ) {
    ...
    system( sprintf( "find %s -name '*_tout.tsv.bz2' -print | sort | parallel '(bunzip2 -vc {} | tail -n +2)' |bzip2 -c > filehash/tt_outs.tsv.bz2", save.touts.file ) )
