...
cluster.tomtom.results <- function( tt.out, seq.type=names(mot.weights)[1],
  ...
  if ( 'e.value1' %in% colnames( tt.out ) ) {
    ...
    cat( nrow(tt.out), "alignments pass the cutoffs.\n" )
    ...
    mot.tab[ 1:length( mot.tab ) ] <- 1:length( mot.tab )
    mot.names.2 <- cbind( paste( tt.out$biclust1, tt.out$motif1, sep="_" ),
                         paste( tt.out$biclust2, abs( tt.out$motif2 ), sep="_" ) )
  } #else {
    #mot.names <- unique( c( as.character( tt.out$query.id ), as.character( tt.out$target.id ) ) )
    #mot.names.2 <- cbind( as.character( tt.out$query.id ), as.character( tt.out$target.id ) )
  #}
  tmp <- matrix( 1, nrow=length( mot.names ), ncol=length( mot.names ) )
  #rownames( tmp ) <- colnames( tmp ) <- mot.names
  tmp[ cbind( mot.tab[ mot.names.2[ ,1 ] ], mot.tab[ mot.names.2[ ,2 ] ] ) ] <- tt.out$p.value
  tmp[ cbind( mot.tab[ mot.names.2[ ,2 ] ], mot.tab[ mot.names.2[ ,1 ] ] ) ] <- tt.out$p.value
  ##tmp[ cbind( mot.names.2[ ,2 ], mot.names.2[ ,1 ] ) ] <- tt.out$p.value
  ##tmp[ tmp > p.value.cutoff ] <- 1 ## shouldnt happen given subsetting above, and takes a long time.
  tmp <- as.dist( tmp )
  cat( "Running hclust...\n" )
  hc <- hclust( tmp, method="average" ) ##"complete" )
  if ( k.cut < 1 ) cs <- cutree( hc, h=k.cut )
  else cs <- cutree( hc, k=k.cut )
  if ( TRUE && k.cut != 1 && require( dynamicTreeCut ) ) { ## use dynamicTreeCut ???
    csd <- cutreeDynamic( hc, k.cut, method='tree', minClusterSize=min.size )
    names( csd ) <- names( cs )
    cs <- csd; rm( csd )
  }
  names( cs ) <- mot.names
  ...
  out.tt <- mc$apply( as.integer( names( sort( table( cs ), decreasing=T ) ) ), function( i ) {
    if ( i == 0 ) return( NULL ) ## No longer for dynamicTreeCut
    ##out.tt <- list()
